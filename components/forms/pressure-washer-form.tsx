...
import { pressureWasherItems, type FormType, type CheckStatus } from "@/lib/types"
...
const itemDescriptions: Record<string, string[]> = {
    ...
    "On/Off Switch": ["Functional and undamaged"],
    "Pull Start Rope": ["Condition and tension"],
    "Fuel Tank": ["No leaks, cap secure"],
    "Fuel levels": ["Check if adequate"],
    "Engine Oil": ["Level and color"],
    "Air Filter": ["Clean and secure"],
    "Guards": ["All in place and secure"],
    "Pressure Hose": ["No cracks or leaks"],
    "Hose Couplings / Quick Coupler": ["Secure and no leaks"],
    "Pressure Gun": ["Trigger and safety functional"],
    "Lance (Wand)": ["Condition and operation"],
}
...
export default function PressureWasherForm() {
    ...
    const [formData, setFormData] = useState({
        ...
        registrationNumber: "",
        date: "",
        documentRefNo: "HSEMS / 8.1.19 / REG / 015B",
        author: "HSE MANAGER",
        revision: "2",
        creationDate: "03/23/20"
    })

    const [items, setItems] = useState<Record<string, CheckStatus>>(
        Object.fromEntries(pressureWasherItems.map((item) => [item, null]))
    )
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const unanswered = pressureWasherItems.filter(item => items[item] === null)
        ...
        try {
            const submission = {
                formType: "pressure-washer-checklist" as FormType,
                formTitle: "Pressure Washer Checklist",
            ...
            exportSubmissionToPDF({
                ...submission,
                id: result.id || `PW-${automaticNumber}`,
            })
            ...
            <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
                <Card className="max-w-md w-full p-8 text-center animate-in fade-in zoom-in duration-300">
                    ...
                    <p className="text-slate-600 mb-8">
                        The Pressure Washer checklist has been recorded.
                    </p>
